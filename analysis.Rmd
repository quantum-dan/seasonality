...
aligned <- ggdraw(plt_box) + draw_plot(plt_map, x=0.35, width=0.65, height=0.43)
  ...
  summarize(
    across(-id,
           list(
             ...
             "outliers" = ~sum(abs((.x - mean(.x, na.rm=T)) / sd(.x, na.rm=T)) >= 3, na.rm=T)
           )),
...
plt <- elev %>%
  ...
  geom_point(size=8) +
  geom_point(aes(elev, value, shape=N < 10, color=Which), data=outliers, size=16) +
  scale_color_discrete(type=c("black", "orange", "green", "purple"), guide=guide_legend(nrow=4,
                                                    title.position = "top",
...
plt <- ggplot(rfit, aes(day)) +
  ...
  scale_x_continuous(expand=c(0, 0), breaks=0:6*60) +
  labs(
    x="Day of Year",
...
plt <- ggplot(tg, aes(x=tslen / 365)) +
  stat_ecdf(size=4) +
  scale_y_continuous(sec.axis = sec_axis(~.*nrow(tg)/1000, name="Gage Count (1000s)")) +
  theme_bw(16) +
  scale_x_continuous(expand=c(0, 0)) +
  labs(x="Gage Timeseries Length (years)",
       y="Cumulative Probability")
# write.img(plt, "Figures/GageECDF", width=4)
ggsave("Figures/GageECDF.png", bg="white", dpi=1200, width=5, height=5.5)
```
...
plt <- ggplot(data, aes(day)) +
  geom_point(aes(y=temperature, color="Observations"),
             alpha=0.1, stroke=NA, size=1) +
  geom_line(aes(y=actemp, color="Three-Sine"), size=1) +
  geom_line(aes(y=tsin, color="Sine"), size=1, linetype="dashed") +
  # scale_color_viridis_d(end=0.8) +
  scale_x_continuous(expand=c(0,0), breaks=0:6*60) +
  scale_y_continuous(breaks=0:8*2) +
  scale_color_discrete(type = c("grey", "black", "orange")) +
  theme_bw(16) +
...
ggsave("Figures/ArkansasDemo.png", width=4, height=5.5, bg="white", dpi=1200)
```
